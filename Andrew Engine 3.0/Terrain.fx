...
cbuffer cbRefractPerFrame
{
	float4 g_f4ClipPlane;
};
             ...
             const OutputPatch<HullOut, 4> quad)
{
	...
	dout.ShadowPosH = quad[0].ShadowPosH;
	
	return dout;
}

[domain("quad")]
DomainOutRefract DSRefract(PatchTess patchTess, 
             float2 uv : SV_DomainLocation, 
             const OutputPatch<HullOut, 4> quad)
{
	DomainOutRefract dout;
	...
	dout.Clip = dot( float4( dout.PosW, 1.0f ), g_f4ClipPlane );
		  ...
		  uniform bool gFogEnabled) : SV_Target
{
	...
	float4 c0 = gLayer1.Sample( samLinear, pin.TiledTex );
	float4 c1 = gLayer2.Sample( samLinear, pin.TiledTex );
	float4 c2 = gLayer3.Sample( samLinear, pin.TiledTex );
	float4 c3 = gLayer4.Sample( samLinear, pin.TiledTex );
	float4 c4 = gLayer5.Sample( samLinear, pin.TiledTex ); 
    ...
    float4 texColor = c0;
    texColor = lerp(texColor, c1, t.r * t.a);
    texColor = lerp(texColor, c2, t.g * t.a);
    texColor = lerp(texColor, c3, t.b * t.a);
 
	//
	// Lighting.
	//

	//float shadowFactor = CalcTheShadowFactor( samShadow, gShadowMap, pin.ShadowPosH );
...
float4 PSRefract(DomainOutRefract pin, 
          uniform int gLightCount, 
		  uniform bool gFogEnabled) : SV_Target
{
	//
...
technique11 LightRefract
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS() ) );
        SetHullShader( CompileShader( hs_5_0, HS() ) );
        SetDomainShader( CompileShader( ds_5_0, DSRefract() ) );
		SetGeometryShader( NULL );
        SetPixelShader( CompileShader( ps_5_0, PSRefract(3, true) ) );
	}
}